...
function run_s7_sweep()
    ...
    # Find the S7 scenario template
    s7_template = findfirst(s -> startswith(s.name, "S7"), cfg.scenarios)
    s7_template === nothing && error("No S7 scenario found")
    s7_base = cfg.scenarios[s7_template]

    total_jobs = length(link_lengths) * seed_count
    @info "S7-only sweep" link_lengths seed_count target_wall nworkers=nworkers() total_jobs
    ...
    for (ll_idx, linklength_km) in enumerate(link_lengths)
        ...
        slack = optimal_slack(linklength_km)
        sim_time, steady_state_start = adaptive_sim_times(linkcapacity; target_wall=target_wall)

        scenario = Scenario(s7_base.name, s7_base.nslots, slack, s7_base.policy, s7_base.cutofftime)

        @info "Starting $(ll_idx)/$(length(link_lengths))" linklength_km linkcapacity slack sim_time
        ...
        @info "  $(scenario.name) @ $(linklength_km) km (slack=$(round(slack, digits=4))): fidelity=$(round(mf, digits=4)), throughput=$(round(mt, digits=4)), skr=$(round(ms, digits=4))"
    end

    # Read existing CSV, drop old S7 rows, append new ones
    outpath = joinpath(out_root, "scenario_sweep.csv")
    existing = CSV.read(outpath, DataFrame)

    # Remove old S7 rows (any name starting with S7)
    filter!(r -> !startswith(r.scenario, "S7"), existing)

    # Add skr columns to existing if missing
    ...
    @info "Done. Updated $(outpath) — removed old S7, added $(length(new_rows)) new rows"
