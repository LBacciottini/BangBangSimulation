...
function run_s8_sweep()
    ...
    # Find the S8 scenario template
    s8_template = findfirst(s -> startswith(s.name, "S8"), cfg.scenarios)
    s8_template === nothing && error("No S8 scenario found")
    s8_base = cfg.scenarios[s8_template]

    total_jobs = length(link_lengths) * seed_count
    @info "S8-only sweep" link_lengths seed_count target_wall nworkers=nworkers() total_jobs
    ...
    for (ll_idx, linklength_km) in enumerate(link_lengths)
        ...
        scenario = s8_base  # S8 has slack=0.0, no per-link override needed
    ...
    # Read existing CSV, append S8 rows
    outpath = joinpath(out_root, "scenario_sweep.csv")
    existing = CSV.read(outpath, DataFrame)

    # Remove old S8 rows if any
    filter!(r -> !startswith(r.scenario, "S8"), existing)
    ...
    @info "Done. Updated $(outpath) — added $(length(new_rows)) S8 rows"
