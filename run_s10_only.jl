...
function run_s10_sweep()
    ...
    s10_template = findfirst(s -> startswith(s.name, "S10"), cfg.scenarios)
    s10_template === nothing && error("No S10 scenario found")
    s10_base = cfg.scenarios[s10_template]

    total_jobs = length(link_lengths) * seed_count
    @info "S10-only sweep" link_lengths seed_count target_wall nworkers=nworkers() total_jobs
    ...
    for (ll_idx, linklength_km) in enumerate(link_lengths)
        ...
        # S10 slack = 80% of S7 optimal slack (reduced by 20%)
        slack = 0.8 * optimal_slack(linklength_km)

        sim_time, steady_state_start = adaptive_sim_times(linkcapacity; target_wall=target_wall)

        scenario = Scenario(s10_base.name, s10_base.nslots, slack, s10_base.policy, s10_base.cutofftime)
    ...
    filter!(r -> !startswith(r.scenario, "S10"), existing)
    ...
    @info "Done. Updated $(outpath) — added $(length(new_rows)) S10 rows"
