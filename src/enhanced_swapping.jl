...
function enhancedfindswapablequbits(net, node, pred_low, pred_high, choose_low, choose_high, chooseslots; agelimit=nothing, policy="OQF")
    reg = net[node]
    low_queryresults  = [
    ...
    error("enhancedfindswapablequbits: unknown policy '$policy'")
...
"""
@kwdef struct EnhancedSwapperProt <: QuantumSavory.ProtocolZoo.AbstractProtocol
    ...
    chooseslots::Union{Vector{Int},Function} = idx->true
    ...
    """policy for choosing which qubits to swap when multiple options are available; currently "RAND" (random), "OQF" (oldest qubit first) and "YQF" (youngest qubit first) are implemented. If ""OQF" or "YQF" is chosen, the `chooseL`, `chooseH`, `chooseslots` functions are ignored."""
    policy::String = "RAND"
end

#TODO "convenience constructor for the missing things and finish this docstring"
function EnhancedSwapperProt(sim::Simulation, net::RegisterNet, node::Int; kwargs...)
    return EnhancedSwapperProt(;sim, net, node, kwargs...)
end

EnhancedSwapperProt(net::RegisterNet, node::Int; kwargs...) = EnhancedSwapperProt(get_time_tracker(net), net, node; kwargs...)

@resumable function (prot::EnhancedSwapperProt)()
    rounds = prot.rounds
    round = 1
    while rounds != 0
        ...
        if isnothing(qubit_pair_)
            if isnothing(prot.retry_lock_time)
                ...
                @debug "EnhancedSwapperProt: no swappable qubits found. Waiting a fixed amount of time..."
        ...
        @debug "EnhancedSwapperProt @$(prot.node)|round $(round): Send message to $(tag1[2]) | message=`$msg1` | time = $(now(prot.sim))"
        # send from here to new entanglement counterpart:
        ...
        @debug "EnhancedSwapperProt @$(prot.node)|round $(round): Send message to $(tag2[2]) | message=`$msg2` | time = $(now(prot.sim))"
